...
Board saves fire every 500ms during active editing. OPFS writes are async but fast — JSON serialization is the only overhead (the async API uses strings, not structured clone). The existing debounce is sufficient — no change needed.
...
Read path (bootstrap):
  localStorage.getItem(key)
    → read from localStorage mirror directly (sync, for initial render)
    → no getSync() method needed — createLocalStorage already reads
      from localStorage synchronously via loadFromLocal()
```
...
| `ohm-drive-synced` | `useDriveSync`   | No       | Session flag. If lost, user just reconnects — the refresh token (which _is_ migrated) is the real value. |
